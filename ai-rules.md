...
- **Run Directory**: Always `cd run/` before invoking `dbprove` to run tests or theorems, unless the user explicitly asks for a different working directory. Use paths like `../out/build/.../dbprove` from there. This keeps generated artifacts such as `logs/`, `proof/`, `table_data/`, and `scratchpad/` under `run/` instead of cluttering the project root. When running with `--docker`, always supply `--query-timeout 120` to prevent theorems from hanging indefinitely.
...
- **Explicit Logging**: Use the plog macros (`PLOGI` for info, `PLOGW` for warnings, `PLOGE` for errors, `PLOGD` for debug) for all informational, diagnostic, and error output in C++ code. Prefer these over `std::cout`/`std::cerr` unless the output is explicitly user-facing terminal content (e.g., theorem listing tables rendered via the `ux::` helpers). The plog macros respect the configured log level and route to both the log file and stdout as appropriate for the current run mode.
...
- **Single-Responsibility Methods**: Keep top-level methods focused on orchestration. If a method starts handling multiple independent responsibilities (for example fetch, transform, and specialized post-processing), extract those responsibilities into clearly named functions.
...
- **ExpressionNode First for Lineage**: For explain work, prefer resolving aliases, join keys, and lineage through resolved `ExpressionNode` structures before adding string-based SQL heuristics. Use textual fallback logic only when the expression graph does not carry enough information.
- **Alias Preference**: For ClickHouse executable SQL reconstruction, prefer a real non-synthetic alias whenever one exists. Only generate or fall back to synthetic aliases when no semantic alias is available and downstream executable SQL still requires a stable name.
- **README Scope**: Keep README files focused on design intent, behavior contracts, invariants, and why the code exists. Do not add validation notes, “tested locally” summaries, implementation diaries, step-by-step wiring guides, or speculative cleanup chatter unless explicitly requested by the user.
