...
POSTGRES_NVME_ROOT="${POSTGRES_NVME_ROOT:-/mnt/nvme}"
PGDATA="${PGDATA:-${POSTGRES_NVME_ROOT}/postgresql/data}"
...
ensure_permissions() {
  mkdir -p "${POSTGRES_NVME_ROOT}" "${PGDATA}"
  chown -R postgres:postgres "${POSTGRES_NVME_ROOT}" /var/lib/postgresql
  chmod 700 "${PGDATA}"
}
...
configure_cluster() {
  ...
  if ! grep -q "^port = ${POSTGRES_PORT}$" "${default_conf}" 2>/dev/null; then
    echo "port = ${POSTGRES_PORT}" >> "${default_conf}"
  fi
...
bootstrap_database() {
  ...
  runuser -u postgres -- "${PG_BIN_DIR}/pg_ctl" -D "${PGDATA}" -o "-c listen_addresses=localhost -p ${POSTGRES_PORT}" -w start
