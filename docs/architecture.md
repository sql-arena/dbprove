...
- [generator lib](#generator) - data generation
- [sql lib](#sql-lib) - connectors and execution methods to various database engines, providing a unified interface
- [runner lib](#runner-lib) - Methods to run, benchmark, measure and analyse queries
...
Merges must occur frequently and long-lived branches are to be avoided. When adding code to `dbprove` expect to 
merge at bi-weekly. You are allowed to merge code that only contains data structures that are needed for later - we
would much rather see your code early, than deal with a large PR later.

Once a phase ends, the spec for that phase gets merged into the documentation files in this directory and into the 
individual `README.md` files the libraries that were changed. The phase ends with a new release that is declared
on GitHub
...
## Theorems
Each **Therom** is a function that can be executed by `main.cpp`. The theorems are
...
The function takes the `std::vector` of theorem names (ex: `{'CLI-0001', 'CLI-0002'}`) and runs their proofs. The 
theorem owns the construction and teardown of `Connection` objects and it uses the passed `engine` and `credentials`
to contruct new connections

If an empty vector is passed, all theorems in the category are proven.

Theorems for a category live in a namespace with the same name as the category.

In addition to `prove` the theorem category must also expose the following function in `prove.h`

```c++
const std::vector<std::string>& list();
```
This returns a vector of all, currently valid theorems. 
