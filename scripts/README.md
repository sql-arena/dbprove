...
- If a script only shells out to a single command without adding real logic,
  prefer deleting it and calling the underlying command directly from the
  caller. That is why the old DuckDB wrapper build script was removed.
- Benchmark output lives under `proof/`, not under `scripts/`.
